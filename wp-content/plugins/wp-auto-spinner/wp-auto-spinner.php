...
Plugin URI: http://valvepress.com/wpautospinner
Description: automatically rewrite your articles to create fresh content for search engines .
Version: 3.0.0
Author: ValvePress
Author URI: http://valvepress.com/
*/ 

/*  Copyright 2012-2013  Wordpress Auto Spinner - Wordpress Rewriter Plugin  (email : sweetheatmn@gmail.com) */
...
function wp_auto_spinner_admin_head() {
         ...
         echo '<script src="'.plugins_url('js/main.wpautospinner.js', __FILE__).'" type="text/javascript"></script>';
}
...
function wp_auto_spinner_admin_edit(){
	wp_enqueue_script(
	'wp_auto_spinner_bulk_spin',plugins_url( '/js/bulk-spin.js' , __FILE__ )
	);
}
...
foreach ($wp_spinner_types as $type){
	...
	add_action('publish_'.$type,'wp_auto_spinner_publish');
